...
const page = () => {
  ...
  useEffect(() => {
    ...
    if (res) {
      ...
      if (current?.projects) setProjects(current.projects);
    }
  }, [id]);

  function handleChange(index: number, key: string, value: string) {
    setProjects(prev => prev.map((item, i) => i === index ? { ...item, [key]: value } : item));
  }

  function handleSave() {
    const res = localStorage.getItem("profile");
    const allProfiles = res ? JSON.parse(res) : [];
    const updated = allProfiles.map((p: any) => p.id === id ? { ...p, projects } : p);
    ...
    <div className="min-h-screen px-4 py-8 flex flex-col items-center">
      <div className="w-full max-w-sm">
          ...
          {projects.map((proj, index) => (
            <div key={index} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <span className="text-white/60 text-xs uppercase tracking-widest">Project {index + 1}</span>
                {projects.length > 1 && (
                  <button onClick={() => setProjects(prev => prev.filter((_, i) => i !== index))} className="text-red-400 text-xs">✕ Remove</button>
                )}
              </div>
              {[{ label: "Project Name", key: "projectName" }, { label: "Live Link", key: "link" }].map((field) => (
                <div key={field.key}>
                  <label className="text-white/70 text-sm mb-1 block">{field.label}</label>
                  <input type="text" value={(proj as any)[field.key]} onChange={(e) => handleChange(index, field.key, e.target.value)}
              ...
              <div>
                ...
                <textarea value={proj.description} onChange={(e) => handleChange(index, "description", e.target.value)} rows={3}
                  className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-white/30 focus:outline-none focus:border-purple-400 transition resize-none" placeholder="Project ke baare mein..." />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={() => setProjects(prev => [...prev, { projectName: "", description: "", link: "" }])}
